...
function IsDecimal(const S: WideString): Boolean;
var
  C: Integer;
begin
  Result := False;
...
function DecodeToInt(const S: WideString): Integer;
var
  C: Integer;
  Percent: Boolean;
  Help: WideString;
...
function DecodeRGB(const S: WideString): Integer;
var
  RS, GS, BS: WideString;
  RGB: WideString;
  R, B, G: Integer;
...
function PrepareHex(const S: WideString): WideString;
var
  C: Integer;
  Help: WideString;
begin
...
begin
  ...
  if SysUtils.TryStrToInt(Color, C) then
  begin
    Result := C;
    Exit;
  end;
